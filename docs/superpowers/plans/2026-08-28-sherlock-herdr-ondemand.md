...
- Remote command invokes: `srun --jobid=JOB_ID --overlap --nodes=1 --ntasks=1 --nodelist=HOST --pty env HERDR_SOCKET_PATH=SOCKET HERDR_BIN --session SESSION`.
...
- Produces: marker `$PWD/herdr-ready`, server log `$PWD/herdr-server.log`, custom connection values `herdr_session` and `herdr_workspace`.
- Starts server with: `HERDR_SOCKET_PATH=SOCKET herdr --session SESSION server`.
...
Install an idempotent trap that first runs `HERDR_SOCKET_PATH="$socket" herdr --session "$session" server stop`, then waits briefly and kills only the recorded server PID if necessary, then calls `release_job_state "$session" "$SLURM_JOB_ID"` and `remove_runtime_dir "$SLURM_JOB_ID" "$runtime_dir"`.

Start the server in the background, poll for at most 120 seconds with:

```bash
HERDR_SOCKET_PATH="$socket" herdr --session "$session" status server --json
```

If the server exits or the timeout expires, print `herdr-server.log` and fail. Once ready, create an initial workspace only when `workspace list` contains no `workspace_id`, using `workspace create --cwd "$workspace" --label "$(basename "$workspace")" --no-focus`. Touch `herdr-ready`, then `wait` for the server process.

- [ ] **Step 5: Implement readiness and connection view**

`after.sh.erb` polls for `herdr-ready` for 120 seconds, exits successfully when present, and calls `clean_up 1` after printing the server log on timeout.
